...
class Drone(object):
    def __init__(self):
        ...
        self._process: Process = Process(target=self.move, daemon=True)
    ...
    def callback(self, event) -> None:
        ...
        print("Callback: ", type(event), type(event_data))
    ...
    def wait_state(self, state):
        while self.__drone_state != state:
            time.sleep(0.05)
    ...
    def set_speed(self, linear_x: float = None, linear_y: float = None, linear_z: float = None, angular_z: float = None) -> None:
        if linear_x is not None: self.__speed_x = linear_x
        if linear_y is not None: self.__speed_y = linear_y
        if linear_z is not None: self.__speed_z = linear_z
        if angular_z is not None: self.__speed_yaw = angular_z
